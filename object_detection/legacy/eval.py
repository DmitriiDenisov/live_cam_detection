...
def main(unused_argv):
  num_run = FLAGS.checkpoint_dir.split('/')[-1]
  temp_path_eval = os.path.join(FLAGS.eval_dir, 'eval_' + num_run)
  if not os.path.exists(temp_path_eval):
    os.makedirs(temp_path_eval)
  else:
    print('Deleting all files in eval folder!')
    filelist = glob.glob(os.path.join(temp_path_eval, '*'))
    for file in filelist:
        os.remove(file)

  FLAGS.eval_dir = temp_path_eval
